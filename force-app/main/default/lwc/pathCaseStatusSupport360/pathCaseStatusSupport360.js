...
export default class PathCaseStatusSupport360 extends LightningElement {
    ...
    handleSubmit() {
        // Handle the submission of the resolution field
        const fields = {};
        fields['Id'] = this.recordId;
        fields[this.fieldApiName] = this.currentValue;
        fields['Resolution__c'] = this.resolution; // Assuming 'Resolution__c' is the API name of the custom field
        ...
        updateRecord(recordInput)
            .then(() => {
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Success',
                        message: 'Record updated with resolution',
                        variant: 'success'
                    })
                );
                this.closeModal();
